...
class TimersTest : ServerTestBase() {
    ...
    @Test
    fun `get returns an empty list when db is empty`() =
        runTest {
    ...
    @Test
    fun `get returns all timers in the database`() =
        runTest {
    ...
    fun `patch requires credentials`() =
        runTest {
            val res =
                client
                    .patch("$timersEndpoint/${UUID.randomUUID()}")
                    .send()
                    .await()
    ...
    @Test
    fun `patch modifies existing timer`() =
        runTest {
            val id = timer1.id
    ...
    @Test
    fun `patch returns 404 when the timer does not exists`() =
        runTest {
