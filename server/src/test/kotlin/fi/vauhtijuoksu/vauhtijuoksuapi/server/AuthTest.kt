...
open class AuthTest : ServerTestBase() {
    ...
    override fun oauthServerPort(): Int = server.url("/").port

    override fun modules(serverPort: Int): List<Module> =
        super.modules(serverPort) +
            listOf(
    ...
    @Test
    fun `Admin discord users can patch`() =
        runTest {
            `when`(playerInfoDb.get()).thenReturn(
                Future.succeededFuture(
                    PlayerInfo("ota tää"),
    ...
    @Test
    fun `Discord users in Vauhtijuoksu server are not admins`() =
        runTest {
            val sessionClient = WebClientSession.create(client)
            val login =
                sessionClient
    ...
    @Test
    fun `Discord users that are not on the Vauhtijuoksu server are not admins`() =
        runTest {
            val sessionClient = WebClientSession.create(client)
            val login =
                sessionClient
