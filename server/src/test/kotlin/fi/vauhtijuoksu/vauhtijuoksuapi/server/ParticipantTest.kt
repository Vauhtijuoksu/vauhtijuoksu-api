...
class ParticipantTest : ServerTestBase() {
    @Test
    fun `get all participants returns an empty list when there are no participants`() =
        runTest {
    ...
    @Test
    fun `add a participant`() =
        runTest {
            val createdParticipant =
                Participant(
                    UUID.randomUUID(),
                    "it's a me",
                    listOf(
