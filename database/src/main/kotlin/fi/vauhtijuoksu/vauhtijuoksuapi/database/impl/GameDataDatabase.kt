                ...
                SqlTemplate
                    ...
                    .flatMap {
                        if (record.participants.isNotEmpty()) {
                            client
                                .preparedQuery(
                                    """INSERT INTO participant_in_game (game_id, participant_id, role_in_game, participant_order) VALUES ($1, $2, $3, $4)""",
                                ).executeBatch(
        ...
        override fun update(record: GameData): Future<Unit> {
            ...
            return pool.withTransaction { client ->
                SqlTemplate
                    ...
                    .flatMap {
                        client
                            .preparedQuery("DELETE FROM participant_in_game WHERE game_id=$1")
                            .execute(Tuple.of(record.id))
                    }.flatMap {
                        if (record.participants.isNotEmpty()) {
                            client
                                .preparedQuery(insertParticipant)
                                .executeBatch(
                                    record.participants.mapIndexed { i, p ->
                                        Tuple.of(
                                            record.id,
